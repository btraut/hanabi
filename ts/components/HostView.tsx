...
class HostViewPage extends React.PureComponent<HostViewProps> {
				...
				<div className="HostView-LinkAndCodeContainer">
					<ol className="HostView-LinkAndCodeContainerList">
						<li className="HostView-LinkAndCodeContainerListItem">
							Visit <span className="HostView-Link">{`${DOMAIN_BASE}/join`}</span>
						</li>
						<li className="HostView-LinkAndCodeContainerListItem">
							Enter the code <span className="HostView-Code">{gameData.code}</span>
						</li>
					</ol>
				</div>
	...
	private _renderWaitingForPhraseSubmissions() {
		...
		return (
			<>
				...
				<ul className="HostView-PlayersContainer">
					{gameData.players.map((player) => {
						const playerSubmitPhrase =
							gameData.phrases[currentRoundIndex] &&
							player.id in gameData.phrases[currentRoundIndex];

						return (
							<li className="HostView-Player" key={player.id}>
								{playerSubmitPhrase && (
									<img className="HostView-PlayerPicture" src="/images/person-done.svg" />
								)}
								{!playerSubmitPhrase && (
									<img className="HostView-PlayerPicture" src="/images/typewriter.svg" />
								)}
								<div className="HostView-PlayerName">{player.name}</div>
							</li>
						);
			...
			<>
				...
				<ul className="HostView-PlayersContainer">
					{gameData.players.map((player) => {
						...
						const playerSubmitPicture =
							gameData.pictures[currentRoundIndex] &&
							player.id in gameData.pictures[currentRoundIndex];
						...
						return (
							<li className="HostView-Player" key={player.id}>
								...
								{!playerSubmitPicture && (
									<img className="HostView-PlayerPicture" src="/images/painting.svg" />
								)}
								<div className="HostView-PlayerName">{player.name}</div>
							</li>
						);
	...
	private _renderReviewingStories() {
		...
		const contributingPlayerOrder =
			(owningPlayerOrder + gameData.presentingRound) % Object.keys(gameData.players).length;
	...
	private _renderGameState(gameData: GameData) {
		switch (gameData.state) {
			case GameState.WaitingForPlayers:
				return this._renderWaitingForPlayers();
