...
} & React.Props<PlayerViewPage> &
	ClientGameManagerPropsAdditions;
...
class PlayerViewPage extends React.PureComponent<PlayerViewProps, PlayerViewState> {
		...
		return (
			<form className="PlayerView-GameForm" onSubmit={this._handleJoinGameSubmit}>
				...
				<div className="PlayerView-GameFormContainer">
					<label className="PlayerView-TextEntryLabel" htmlFor="PlayerView-Code">
						Code:
					</label>
					<input
						...
						ref={(input: HTMLInputElement | null) => {
							this._joinGameCodeInput = input;
						}}
						autoCorrect="off"
						autoCapitalize="none"
					/>
					<label className="PlayerView-TextEntryLabel" htmlFor="PlayerView-Name">
						Name:
					</label>
					<input
						className="PlayerView-TextEntryInput"
		...
		return (
			<>
				...
				<div className="PlayerView-CanvasContainer" ref={this._setDrawPlayerPictureCanvasSize}>
					{drawPlayerPictureCanvasSize && (
						<Canvas
							ref={(ele: Canvas | null) => {
								this._drawPlayerPictureCanvas = ele;
							}}
				...
				<button className="PlayerView-SubmitButton" onClick={this._handleDrawPlayerPictureSubmit}>
					Submit
				</button>
			</>
		);
	...
	private _renderWaitingForOtherPlayers() {
		return (
			<>
				...
				<p className="PlayerView-BodyText">
					You can start when everyone has finished drawing themselves.
				</p>
			</>
		);
	}
	...
	private _renderWaitingForStart() {
		...
		return (
			<>
				...
				<button className="PlayerView-SubmitButton" onClick={this._handleStartGameButtonClick}>
					Everyone’s ready
				</button>
			</>
		);
	...
	private _renderEnterPhrase() {
		...
		return (
			<form className="PlayerView-GameForm" onSubmit={this._handleEnterPhraseSubmit}>
				...
				<div className="PlayerView-GameFormContainer">
					<label className="PlayerView-TextEntryLabel" htmlFor="PlayerView-Phrase">
						Phrase:
					</label>
					<input
						className="PlayerView-TextEntryInput"
	...
	private _handleEnterPhraseSubmit = (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (this._enterPhraseInput) {
			clientGameManager.enterPhrase(
				gameData.code,
				gameData.currentRound,
				this._enterPhraseInput.value,
			);
		}
	};
		...
		return (
			<>
				...
				<div className="PlayerView-CanvasContainer" ref={this._setDrawPictureCanvasSize}>
					{drawPictureCanvasSize && (
						<Canvas
							ref={(ele: Canvas | null) => {
								this._drawPictureCanvas = ele;
							}}
				...
				<button className="PlayerView-SubmitButton" onClick={this._handleDrawPictureSubmit}>
					Submit
				</button>
			</>
		);
	...
	private _renderWaitingForOtherPlayersToSubmitPictures() {
		return (
			<>
				<h1 className="PlayerView-Title">Waiting for others…</h1>
				<p className="PlayerView-BodyText">
					You’ll move on when everyone has finished drawing a picture.
				</p>
			</>
		);
	}
		...
		return (
			<>
				...
				<div className="PlayerView-GameActions">
					<button className="PlayerView-SubmitButton" onClick={this._handleAdvanceStoryReviewClick}>
						View Escape
					</button>
				</div>
			</>
	...
	private _renderReviewingStories() {
		...
		return (
			<>
				<h1 className="PlayerView-Title">Let's review.</h1>
				<p className="PlayerView-BodyText">
					Viewing {presentingPlayer.name}: Round {gameData.presentingRound + 1}
				</p>
				<div className="PlayerView-GameActions">
					<button className="PlayerView-SubmitButton" onClick={this._handleAdvanceStoryReviewClick}>
						Next
	...
	private _renderPlayAgainOptions() {
		...
		return (
			<>
				...
				<div className="PlayerView-GameActions">
					<button className="PlayerView-SubmitButton" onClick={this._handlePlayAgainClick}>
						Play Again
					</button>
					<button className="PlayerView-SubmitButton" onClick={this._handleEndGameClick}>
						End Game
					</button>
				</div>
			</>
	...
	private _renderGameState() {
		...
		switch (gameData!.state) {
			case GameState.WaitingForPlayers:
				return this._renderInGameLobby();
			case GameState.WaitingForPhraseSubmissions:
				return this._renderWaitingForPhraseSubmissions();
			case GameState.WaitingForPictureSubmissions:
				return this._renderWaitingForPictureSubmissions();
			case GameState.WaitingToReviewStories:
				return this._renderWaitingToReviewStories();
			case GameState.ReviewingStories:
				return this._renderReviewingStories();
			case GameState.PlayAgainOptions:
				return this._renderPlayAgainOptions();
		}
	...
	public render() {
		...
		return <div className="PlayerView">{viewContent}</div>;
	}
}
