import { HANABI_BOARD_SIZE, HANABI_DEFAULT_TILE_PADDING, HANABI_TILE_SIZE, } from './HanabiGameData.js';
export function getSlotXForDraggingTile(x, max = Number.MAX_SAFE_INTEGER) {
    const slot = Math.floor((x + HANABI_TILE_SIZE.width / 2) / (HANABI_DEFAULT_TILE_PADDING + HANABI_TILE_SIZE.width));
    return Math.max(Math.min(slot, max), 0);
}
export function isTileInTopHalf(position) {
    return position.y < HANABI_TILE_SIZE.height + HANABI_DEFAULT_TILE_PADDING;
}
export function getNewPositionsForTiles(draggingTile, otherTilePositions, includeDraggingTile = true) {
    const topTiles = [];
    const bottomTiles = [];
    let maxZIndex = 0;
...
export function getPositionInContainer(originalPosition, delta) {
