...
// What is this? This is the horrible (and horribly out-of-date) code that uses
// signalhub to negotiate WebRTC connections. I copied it in here so I could run
// it through prettier and read it easier than on GitHub.
...
function WebRTCSwarm(hub, opts) {
	if (!(this instanceof WebRTCSwarm)) {
		return new WebRTCSwarm(hub, opts);
	}
	if (!hub) {
		throw new Error('SignalHub instance required');
	}
	if (!opts) {
		opts = {};
	}
...
function setup(swarm, peer, id) {
	...
	function kick() {
		if (swarm.closed || sending || !signals.length) {
			return;
		}
		sending = true;
...
function connect(swarm, hub) {
	if (swarm.closed || swarm.peers.length >= swarm.maxPeers) {
		return;
	}
